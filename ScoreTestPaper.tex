...
{Explanation of how to calculate score test statistics. Then go into
  benefits of RTMB.}

\subsection{Issues with nesting}
\label{sec:nesting-issues}

In our example, issues with nesting arise as we consider models that allow for individual
heterogeneity. As shown in \textbf{Table (results)}, our candidate set
contains models that a two-class finite mixture model for $g_0$ and/or
...
placing fixed equality constraints on any additional parameters. Any small changes in
the values of these constrained parameters must change the
likelihood. This ensures the $H_1$ log-likelihoods have curvature at any
$H_0$ estimates, allowing us to calculate the required gradients. 



% For two models to be nested appropriately for score tests, we must be
% able to obtain the simpler model from the more complex one by placing fixed
% constraints on each of the latter model's parameters. 
...
% For two models to be nested, it must be possible to obtain
% the simpler model from the more complex one by placing constraints on
% the latter model's parameters. More specifically, there must be a
% point in the parameter space for both models that returns the same
% likelihood value.

% Therefore, models with mixture
% components can only be nested under, and therefore compared to,
% models with matching mixture components. Consider the example where
% our $H_0$ model has no mixture component 
% and our $H_1$ model has a mixture component for $g_0$. 

% there must be a
% point in the parameter space for both models that provides the same
% likelihood value. In other words, there must be a point in the
% parameter space at which the larger model (the model specified under
% $H_1$) has the same parameter restrictions as the smaller model (the
% model under $H_0$). 

% For example, say our
% $H_0$ model has no mixture component and our $H_1$ model has a mixture
% component for $g_0$. 
