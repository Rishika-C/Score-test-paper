...
nesting structure in the candidate set, making it unclear how to
organise models into levels as described above.
...
for SCR model selection. Using data
collected from a mist-netting survey on the Patuxent Research Refuge
in Maryland, USA by C.S. Robbins from 1961 to 1972, a variety of SCR
...
selection via AIC remains an accessible, and therefore common, approach.
...
set consists of all adult ovenbirds that were caught, with one bird
...
parameter dependencies presented, our set contains forty five models. 

To construct an accurate picture of the potential benefits afforded by
the score test framework, we fit all models in our candidate set using
the `secr' R package and select the best model via AIC. Then, we
implement the score test framework to select the final model. We
compare features of both approaches, including the time taken and any
differences in the chosen model.

\textbf{(Are not mentioning that 1961-1972 data set focused on
  Red-eyed vireo, not ovenbirds. Doesn't seem relevant. Okay?)}
