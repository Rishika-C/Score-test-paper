...
day. In the context of SCR, each year represents one session. As
is standard when fitting multi-session SCR models, we treat samples from each session as if
...
Working with forty five SCR models, we extend the score test
...
Following the example set by \textcite{borchersEfford2008}, and in
parlance used by the `secr' R package, we
consider models with the dependencies shown in Table
\ref{table:dependencies}. As described in 
\textbf{(cite SECR book)}, the parameter $D$ describes population
density, while $g_0$ and $\sigma$ describe the probability of
individuals being caught during the survey.

\begin{table}[h!]
  \centering
\begin{tabular} { |c|c|c|c|  }
  \hline
  $D$ dependencies & $\sigma$ dependencies &
                                             \multicolumn{2}{|c|}{$g_0$ dependencies}
  \\
  \hline
  $D \sim 1$ & $\sigma \sim 1$  & $g_0 \sim 1$ & $g_0 \sim b$ \\
  \hline
  $D \sim \text{session}$ & $\sigma \sim \text{session}$ & $g_0 \sim
                                                           \text{session}$
             &  $g_0 \sim \text{session} + b$ \\
  \hline
...
In Table 1:
\begin{itemize}
  \item Brief description of each dependency, e.g. 1 means constant
    across all sessions
\end{itemize}


% {Description of candidate model set, dependencies.}
